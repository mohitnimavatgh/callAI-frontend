...
const getQuickQuestion = () => {
  loader.loading = true
  quickQuestions.list(quickQuestionParams.value).then((res) => {
  ...
  <div class="mt-5">
    ...
    <div class="box dark:p-0 mt-3 lg:w-2/3 w-full bg-white dark:bg-gray-800">
        ...
        <ul class="grid w-full gap-3 md:grid-cols-2">
          <li v-for="(item, index) in items" :key="index" class="mt-2">
            <FormRadio :id="`radio-${index}`" name="access" :value="item.value"
              v-model="v$.quickQuestion.access_type.$model" :errors="v$.quickQuestion.access_type.$errors" />
      ...
      <div class="my-4 flex justify-end">
        <Button text="Add Quick Question" v-if="!QuickQuestionUpdate" @click="createQuickQuestion" frontIcon="fas fa-plus"
          class="m-0" />
    ...
    <div class="box mt-5 dark:p-0 bg-white dark:bg-gray-800">
      ...
      <Pagination
        v-if="quickQuestionLists && quickQuestionLists.total && quickQuestionLists.per_page && quickQuestionLists.total > quickQuestionLists.per_page"
        class="mt-4 flex justify-end" :totalRecords="quickQuestionLists.total" :currentPage="quickQuestionParams.page"
        :recordsPerPage="quickQuestionLists.per_page" @pageChange="quickQuestionPageChange" />
    </div>
